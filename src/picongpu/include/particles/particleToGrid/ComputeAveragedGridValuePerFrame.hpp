/**
 * Copyright 2013-2016 Marco Garten, Axel Huebl, Heiko Burau, Rene Widera
 *
...
HDINLINE float1_64
ComputeAverageGridValuePerFrame<T_ParticleShape, T_DerivedAttribute>::getUnit() const
{
...
ComputeAverageGridValuePerFrame<T_ParticleShape, T_DerivedAttribute>::getUnitDimension() const
{
...
template<class T_ParticleShape, class T_DerivedAttribute>
HINLINE std::string
ComputeAverageGridValuePerFrame<T_ParticleShape, T_DerivedAttribute>::getName() const
{
    return T_DerivedAttribute().getName();
}
/** Functor implementation
 *
 * \tparam T_ParticleShape type of particle shape
 * \tparam
 */
template<class T_ParticleShape, class T_DerivedAttribute>
template<class FrameType, class TVecSuperCell, class BoxTmpResult, class BoxTmpCounter >
DINLINE void
ComputeAverageGridValuePerFrame<T_ParticleShape, T_DerivedAttribute>::operator()
(FrameType& frame,
 const int localIdx,
 const TVecSuperCell superCell,
 BoxTmpResult& tmpResultBox,
 BoxTmpCounter& tmpCounterBox)
{
    ...
    for (int i = 0; i < numWriteCells; ++i)
    {
        ...
        /* calculate overlap of particle shape with local cell */
        const float_X assignComb = assign.productOfComponents();

        /** add the doubly weighted attribute to the FieldTmp-value of the local cell
         * 1st: value is weighted with the overlap of particle shape and cell
         * 2nd: value is multiplied by the particle weighting for later averaging
         * over the "real" particle number
         */
        atomicAddWrapper(&(fieldTmpShiftToParticle(offsetToBaseCell).x()),
                         assignComb * particleAttr * particle[weighting_]);
    }
}
